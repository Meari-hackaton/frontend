// src/pages/connect/ConnectLayout.jsx
...
export function CTAButton({ children, onClick }) {
  return (
    <button
      onClick={onClick}
      className="w-[320px] sm:w-[360px] py-4 rounded-full bg-[#5b86ff]
                 text-white font-semibold tracking-wide
                 shadow-[0_14px_32px_rgba(70,100,255,.28)]
...
export default function ConnectLayout({
  eyebrow = "",          // 상단 작은 제목(정보 연결/경험 연결/지원 연결)
  title,                 // 큰 타이틀(“새로운 기회를 잡아보세요” 등)
  children,              // 카드 내부 컨텐츠
  ctaText = "바로가기",
  onCta,                 // CTA 클릭
  showBack = true,
  ...
  return (
    <div className="relative min-h-screen overflow-hidden">
      ...
      <header className="relative z-10 text-center pt-24">
        {eyebrow && <div className="text-blue-500 mb-3">{eyebrow}</div>}
